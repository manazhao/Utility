...
$tmp_result = $cluster_manager->execute_on_cluster(
	cmd_pattern => "[ -f " . LEAF_CATEGORY_DIR . "/category_item_profile_%s.json ] && echo 1",
	cmd_args => $LETTER_SEQ
);
...
if(not $cluster_manager->all_nodes_true($tmp_result)){
	$cluster_manager->sync_cluster_execute(
		cmd_pattern => join(";" ,
			( 
				"ProcessCrawledPage.pl --wd=LeafCategory --target=x%s --parser=Amazon/CategoryPageParser 1>/dev/null 2>&1",
				"ExtractCategoryItemProfile.pl --input-file=LeafCategory/x%s_parsed.json --output-file=LeafCategory/category_item_profile_%s.json",
			)
		),
		cmd_args => [$LETTER_SEQ,$LETTER_SEQ,$LETTER_SEQ]
	);
}else{
...
if(not $cluster_manager->node_file_exist($main_node, LEAF_CATEGORY_DIR . "/category_item_profile.json")){
	my $merged_file_tmp = LEAF_CATEGORY_DIR . "/category_item_profile.json.tmp";
	my $merged_file = LEAF_CATEGORY_DIR . "/category_item_profile.json";
	$cluster_manager->cluster_cat(
		remote_path_pattern => LEAF_CATEGORY_DIR . "/category_item_profile_%s.json",
		remote_path_args => $LETTER_SEQ,
		local_path => $merged_file_tmp
	);
	# remove duplicate json objects
	$cluster_manager->execute_on_local("UniqueJSON.pl < $merged_file_tmp > $merged_file 2>LeafCategory/UniqueJSON.pl.log");
...
if(not $cluster_manager->all_nodes_true($cluster_manager->execute_on_cluster( cmd_pattern => "[ -f " . LEAF_CATEGORY_DIR . "/x%s_asin_rp.csv ] && echo 1", cmd_args => $LETTER_SEQ))){
	...
	$cluster_manager->sync_cluster_execute(
		cmd_pattern => "GenerateProductReviewURL.pl < " . LEAF_CATEGORY_DIR . "/category_item_profile_%s.json > " . LEAF_CATEGORY_DIR . "/x%s_asin_rp.csv",
...
my $asin_file = PRODUCT_DIR . "/asin.csv";
if(not $cluster_manager->node_file_exist($main_node, PRODUCT_DIR . "/api_response_linked.csv") ){
	print ">>> start to query amazon item profile on node-0\n";
	my $response_offset_file = PRODUCT_DIR . "/api_response_pos.csv";
	my $response_linked_file = PRODUCT_DIR . "/api_response_linked.csv";
	my $log_file = PRODUCT_DIR . "/nohup_api.log";
	my $cmd = "nohup crawler_console queryItem --asin-file=$asin_file --offset-file=$response_offset_file --response-file=$response_linked_file";
...
my @bk_file_list = (
	...
	PRODUCT_DIR . "/api_response_linked.txt",
	PRODUCT_DIR . "/api_response_pos.csv",
